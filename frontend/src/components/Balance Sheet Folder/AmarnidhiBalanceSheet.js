...
const RaseedBalanceSheet = () => {
  ...
  const fetchRecords = async () => {
    try {
      const response = await fetch(`/api/amarnidhi/balance?year=${year}${month ? `&month=${month}` : ''}`);
...
export default RaseedBalanceSheet;
